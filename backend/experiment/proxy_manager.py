...
class SmartProxyManager:
    ...
    def _load_proxies(self):
        """Load proxies from environment or Webshare file"""
        ...
        # Try Webshare proxy file if no environment proxies
        if not self.proxies:
            proxy_files = ["Webshare-10-proxies.txt", "proxies.txt", "proxy_list.txt"]
                        ...
                        for line in proxy_lines:
                            if line.strip():
                                ...
                                if len(parts) >= 4:
                                    ...
                                    ip, port = parts[:2]
                                    server = f"http://{ip}:{port}"
                                    self.proxies.append(ProxyInfo(server=server))
                        
                        logger.info(f"✅ Loaded {len(self.proxies)} proxies from {proxy_file}")
                        return
                        
                    except Exception as e:
                        logger.warning(f"⚠️ Error loading from {proxy_file}: {e}")
                        continue
        
        if not self.proxies:
            logger.warning("⚠️ No proxies loaded, running without proxy rotation")
        else:
